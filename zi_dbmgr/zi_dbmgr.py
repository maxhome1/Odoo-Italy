...
class zi_dbmgr_db_config(orm.Model):
    ...
    def _init_conf (self):
        ...
        conf_file = addons.get_module_resource('zi_dbmgr', 'conf', 'zi_dbmgr.ini')
        if conf_file:
        ...
        if cfg_obj.getboolean(s, "store_into_db"):
            ...
            for conf in self.browse(cr, uid, ids, context=None):
                ...
                if conf.new_db_name_rule=='GTIN':
                    name = name+self.eval_GTIN(cr, uid, conf.new_db_name_next_count, context)
                else:
                    name = name+str(conf.new_db_name_next_count).zfill(conf.new_db_name_count_lenght)
    ...
    _columns = {
            ...
            'new_db_name_count_lenght': fields.integer('Name - Lengh Count', required=True),
            ...
            'new_db_name': fields.function(_new_db_name_get, type='char', string='Name Next Database', readonly=True),
    ...
    def eval_GTIN(self, cr, uid, last_accid, context=None):
        user_acc='%07d' % last_accid                            # pad 7 digits
        chkdgt = 0
        for i in range(7):
           n=int(user_acc[i])
           if (i % 2):
    ...
    def increase_count(self, cr, uid, ids, context=None):
        ...
        for config in self.browse(cr, uid, ids):
            val = {
                'new_db_name_next_count' : config.new_db_name_next_count + 1 or False
                }
...
#                                           params.get('user_password'))           
        ...
        # Increase Count
        service_db_config_obj.increase_count(cr, uid, [config.id], context=None)
