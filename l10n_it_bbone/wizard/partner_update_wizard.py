...
#        pdb.set_trace()
        ...
        if n_partners > 0: 
            ...
            for i in range(n_partners):
                partner_obj = pooler.get_pool(cr.dbname).get('res.partner').browse(
                    cr, uid, partners[i])
                if partner_obj:
                    ...
                    if state_id:
                        state_obj = pooler.get_pool(cr.dbname).get('res.country.state').browse(
                            cr, uid, state_id[0])
                        if state_obj:
                            if update_country:
                                vals = {'state_id': state_obj.id, 'country_id':country_to_find}
                            else:    
                                vals = {'state_id': state_obj.id}
                            id_upd = [partner_obj.id]
                            p.write(cr, uid, id_upd, vals, context=context)
                            log += _('Updated: ')+partner_obj.name +' '+partner_obj.zip +' '
                            log += partner_obj.city+' '+ state_obj.code+' \n'
    
            log += _('Updated Partners: ') + str(n_partners)
            
            cpy = pooler.get_pool(cr.dbname).get('res.company').search(cr, uid, 
                ['|', ('id', '=', user.company_id.id), 
                ('id', 'child_of', user.company_id.id)])
            c = pooler.get_pool(cr.dbname).get('res.company')
            for i in range(len(cpy)):
                c.write(cr, uid, cpy[i], {'it_partner_updated': True}, context=context)
                
        else:
            log += _('No Partner needs to be updated.') 

        self.write(cr, uid, ids, {'state': 'step2', 'log': log}, context=context)
